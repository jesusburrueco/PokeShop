...
export class ProductCrudComponent implements OnInit {
  ...
  addNewProduct() {
    ...
    this.product_service.allProduct().subscribe(products => {
      const lastId = products.reduce((maxId, product) => Math.max(maxId, product.id), 0); // Encuentra el mayor ID
      const newId = (lastId + 1).toString(); // Incrementa el ID
